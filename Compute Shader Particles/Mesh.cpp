...
void Mesh::CreateBuffers(Vertex* vertArray, int numVerts, unsigned int* indexArray, int numIndices, ID3D11Device* device)
{
	...
	// Save the indices
	this->indicesArray = new unsigned int[numIndices];
	std::copy(indexArray, indexArray + numIndices, this->indicesArray);
	this->numIndices = numIndices;
	this->vertexArray = new Vertex[numVerts];
	std::copy(vertArray, vertArray + numVerts, this->vertexArray);
	std::cout << " local " << this->numIndices << " injected " << numIndices << std::endl;
}
